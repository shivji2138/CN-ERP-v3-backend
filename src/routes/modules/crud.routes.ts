...
import type { PermissionResource } from '../../types/auth.js';
...
for (const [path, resource, model] of modules) {
  ...
  router.get('/', requirePermission(`${resource as PermissionResource}:read`), controller.list);
  router.post('/', requirePermission(`${resource as PermissionResource}:create`), controller.create);
  router.get('/:id', requirePermission(`${resource as PermissionResource}:read`), controller.getById);
  router.patch('/:id', requirePermission(`${resource as PermissionResource}:update`), controller.update);
  router.delete('/:id', requirePermission(`${resource as PermissionResource}:delete`), controller.remove);
